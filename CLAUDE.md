...
- **Request blocked 脱敏恢复**: Codex policy block 仍走一次性的 Message Edit 分支；原生 `thread/fork` 只能切 completed turn 边界，不能伪造 turn 内 tool result。恢复前从 DB 读取最后一段尚未被 assistant/user 消费、且 ID 连续的 `tool_result` batch，把原文只交给独立 Apex GLM `glm-5.3` sanitizer。sanitizer prompt 明确 tool 内嵌指令不可信；返回后再本地过滤密钥，以 `[CCM sanitized tool-output summary]` 附加到重放请求，并写明原始 log ID、大小、hash 与“不要为找回原文重跑副作用命令”。GLM 账号只从 enabled 且非 retired 的 Apex 账号选择并读取受管 key；摘要失败则回退原有纯 Edit replay。第二次 blocked 必须停住，不得无限恢复。

...
- **原生 Goal 普通消息隔离**: Codex 原生 Goal 为 `paused` 或 `blocked` 时，普通聊天消息只允许开启独立 `turn/start`，不得隐式 `thread/goal/set active` 或 steer 到 Goal continuation；否则一个普通问题会重新启动自治循环并刷出重复 blocked 提醒。Goal 只能通过明确的 Goal 操作恢复。`active` Goal 的权威 turn 收养与 descendant 闸门保持原逻辑。

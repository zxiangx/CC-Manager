...
- **原生 Goal 恢复边界**: 用户 Stop 后持久化为 `paused` 的 Codex 原生 Goal，将下一条 Standard 消息视为明确恢复意图：CCM 先建立 owner，再 `thread/goal/set active` 并把消息 steer 到同一个 continuation。`blocked` Goal 必须保持 inactive，普通消息只开启独立 `turn/start`，不得重新激活自治循环或重复刷出 blocker。`active` Goal 的权威 turn 收养与 descendant 闸门保持原逻辑。

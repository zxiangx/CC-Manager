...
- **Markdown 数学公式**: 所有聊天与 Discussion 的 Markdown 必须统一经 `components/Markdown/MarkdownRenderer.tsx` 渲染。公式支持 `$$...$$` 及 Codex 常见的 `\\(...\\)` / 整段 `\\[...\\]`；单 `$...$` 明确保留为普通文本，避免价格等货币内容误判。反斜杠分隔符必须在 Markdown AST 上按 text node / 整段 paragraph 转换，跳过链接、图片、HTML、definition 和各类 code，禁止 raw source 全局改写；模型误输出 `# \\[` / `### $$` 时，只允许把“纯公式 opener heading + 相邻闭合正文”这一严格 AST 形态恢复为 display math，含文字的真实 heading 不得降级；KaTeX 必须保持 `trust: false`、有界 `maxSize`，直接依赖版本须与 `rehype-katex` 使用的版本一致
- **Task 自动排序**: `auto_sort_on_access` 是兼容保留的字段名；开启时语义是“标星组优先，组内按逻辑会话最后一条非空 user/assistant `message` 时间倒序”，不得再用 `last_accessed_at` 或 `sort_order` 干扰。编辑消息产生的隐藏 Task 全部聚合到 canonical Task。关闭时才使用手动 `sort_order`，再回退 `created_at`。
...
- **本地版本一键部署**: 禁用远端更新时，管理员顶栏仍显示“部署本地版本”。`POST /api/system/deploy` 只调用同一 `start_repair` 安全事务部署磁盘现有 commit，不得 fetch/pull/checkout；活跃 Task/Instance/Monitor/Sub-Agent 继续 fail closed 阻止停服。前端经 `system_update` 与 status/health 轮询展示步骤，服务恢复并出现可验证 terminal 后才提示成功。
- **原生 Goal 控制与恢复边界**: `paused` 是保留目标、进度与用量的显式控制状态，不是删除。普通 Standard 消息不得重新启用 `paused`、`blocked`、usage/budget limited 或 complete Goal；只有用户 Goal 面板或主 Agent 的 task-scoped `ccm_resume_goal` 明确启用时，CCM 才先建立 owner，再 `thread/goal/set active`、等待 exact `turn/started` 并 steer 一条不可见控制续跑指令。主 Agent 可用 `ccm_update_goal` 修改目标文本但不改变状态或清除 Goal；新 Goal 仍由 Codex 原生 `create_goal` 创建。用户暂停先走 Task Stop 终止当前 Goal turn；Agent 的 `ccm_pause_goal` 只持久化 paused、让当前 turn 正常收尾，随后不得继续自动 push。Agent 无删除工具；用户仍可永久 clear。跟随 Goal 的 turn 若因 provider stream 短暂断开被 Codex 写成 `blocked`，CCM 必须先规范化为 `paused`，重试时再走显式恢复；重试耗尽也要保留 paused。普通 turn 旁本来就 blocked 的 Goal 没有 generation 证据，不得被误恢复。request-block 隔离、自动 compact 或 context overflow 必须在替换 thread 前把 active/paused/blocked Goal 以 Task metadata handoff 保存，在 fresh thread 建立 owner 前先种成 paused；仅原来应继续的 Goal 才恢复 active，持久 handoff 只在新 turn 已被 CCM 跟踪后以 CAS 清理。`active` Goal 的权威 turn 收养与 descendant 闸门保持原逻辑。
...
- **Codex 动态 Plan 清单（2026-08-08）**: 当前 app-server v2 的权威快照事件是 `turn/plan/updated`，payload 为 `plan: [{step,status}]`，禁止凭旧 exec JSON 猜成 `item/updated + todoList`。adapter 将其转换成带 exact turn identity 的 `todo_list` 事件；同一 turn 用稳定 `todo_id` 覆盖上一份 durable snapshot，并以新 log id 重新插入以留在最新历史页。WebSocket 与 HTTP 都必须返回结构化 `todo_items`，前端按 `todo_id` 原位替换同一张 Plan 卡片；`pending/inProgress/completed` 规范化为 `pending/in_progress/completed`，刷新后仍显示最后快照。旧 `codex exec --json` 的 `todo_list` item 继续兼容。
...
- **Codex 消息编辑分支**: 编辑已发送消息绝不改写 `LogEntry` 或 rollout，而是复用 Fork 建立新的 Task/thread。`message_branches` + `message_branch_versions` 持久化同一消息位置的有序版本；原 Task/消息作为第 1 版，新 Task 的 seed 草稿发送后才原子绑定其真实 user log。前端在已落库的普通/`source=inject` 人类用户消息及初始 Prompt 上开放铅笔入口，Monitor/Sub-Agent 等生成消息仍禁止。普通消息使用精确 turn 边界；注入是 same-turn steer，原生协议没有 item fork，因此必须唯一映射 containing turn、Fork 到其前一 completed turn，并把该 turn 在注入前的用户输入作为一次性隐藏 replay prefix，无法唯一证明时 409，禁止猜测。左右箭头切换真实 Task，因此每个版本继续对话时使用各自独立 context。ordinal > 0 的实现 Task 必须从普通 Task list/count 隐藏，只能经分支箭头和 direct ACL API 访问，避免侧栏重复 Session；普通 Fork 必须清除继承的 branch metadata，不能伪装成原分支版本。
...
- **Request blocked 脱敏恢复**: Codex policy block 仍走一次性的 Message Edit 分支；原生 `thread/fork` 只能切 completed turn 边界，不能伪造 turn 内 tool result。自动流程不要猜测单个敏感 result：选择 block 前最后一个 user/assistant message 边界之后的所有 `tool_result`（串行 tool call 之间会夹 `tool_use`，因此 result ID 不必连续），整体交给独立 Apex GLM `glm-5.3` sanitizer；若该批次之后已有 assistant/user 消费消息，则回退纯 Edit replay，不猜旧输出。sanitizer prompt 明确 tool 内嵌指令不可信；返回后再本地过滤密钥，以 `[CCM sanitized tool-output summary]` 附加到重放请求，并写明原始 log ID、大小、hash 与“不要为找回原文重跑副作用命令”。GLM 账号只从 enabled 且非 retired 的 Apex 账号选择并读取受管 key；摘要失败则回退原有纯 Edit replay。第二次 blocked 必须停住，不得无限恢复。
...
- **瞬时 429/过载自动等待重试**: Anthropic **基础设施侧**的临时限流/过载（CLI 文案 `Server is temporarily limiting requests (not your usage limit)` / overloaded，是 Anthropic 官方报错而非 CCM 内部），换号无用 → 退避后用**同一账号** `--resume` 重试。检测器 `is_transient_overload`（`claude_pool.py`，先排除 `is_rate_limited`/`is_auth_failure` 保证与「额度用尽要换号」互斥），退避 `transient_retry_delay`（指数+jitter）。普通 transient 仍受 `transient_retry_max` 限制；Codex 精确的 `Selected model is at capacity` 由 `is_codex_capacity_error` 窄化识别，固定等待 `codex_capacity_retry_delay`（默认 60s）后同号无限重试，直到成功或用户停止，绝不能因通用预算耗尽判失败。开关/参数：`transient_retry_enabled`(默认 True)、`transient_retry_max`(5)、`transient_retry_base_delay`(10s)、`transient_retry_max_delay`(120s)、`codex_capacity_retry_delay`(60s)。**关键陷阱**：PTY 模式下 api_error 中止 turn 但**持久 session 仍存活 → exit_code 报 0**，单看退出码会误判成功；故 instance_manager 在 `_process_event` 里对带 `is_error` 且命中检测器的事件打 **turn-scoped 标记** `_transient_seen`（`launch()` 重置、`transient_error_seen()` 读取），dispatcher 据此即便 exit_code=0 也触发重试。**标记必须只认当前前台 turn 的活事件**：`_process_event` 打标前要排除 `orphan`（resume 时 PTY 重读 JSONL 回放的上一 turn 旧 api_error）和 `autonomous`（后台子 agent turn 的报错）事件——否则成功 resume 的 turn 会被旧错误重新置标，`still_transient` 永真→烧光重试预算→任务被误判 failed（recover-then-failed bug，task #729）。Autonomous 任务走 dispatcher `_run_transient_retry`（迭代自驱，容量故障长期持续也不增长调用栈）；chat 子进程模式走 instance_manager `_try_chat_transient_retry`（`_consume_output.finally` 自驱循环），PTY 模式由 `_process_queued_message` 在 `_wait_process` 后用 while 循环驱动（heartbeat 覆盖、不会被看门狗误杀）。重试与号池轮换单向衔接（普通 transient 用尽→轮换；轮换不回切 transient），无 ping-pong

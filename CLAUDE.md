...
- **Codex 消息编辑分支**: 编辑已发送消息绝不改写 `LogEntry` 或 rollout，而是复用精确 Fork 建立新的 Task/thread。`message_branches` + `message_branch_versions` 持久化同一消息位置的有序版本；原 Task/消息作为第 1 版，新 Task 的 seed 草稿发送后才原子绑定其真实 user log。前端只在普通、已落库、非 inject/Monitor/Sub-Agent 的用户消息及初始 Prompt 上开放铅笔入口，左右箭头切换真实 Task，因此每个版本继续对话时使用各自独立 context。ordinal > 0 的实现 Task 必须从普通 Task list/count 隐藏，只能经分支箭头和 direct ACL API 访问，避免侧栏重复 Session；普通 Fork 必须清除继承的 branch metadata，不能伪装成原分支版本。

...
- **Codex Goal 持久控制** — 普通消息不改变 Goal 状态；用户或主 Agent 可显式暂停、恢复和修改目标，Agent 无删除权限。Goal turn 的短暂断流不会永久误标为 `blocked`，安全换 thread 时也会把仍可继续的 Goal 交接到同一 CCM Task
- **Codex 动态计划清单** — Codex 调用 `update_plan` 时，聊天中显示与桌面端一致的 Plan 卡片，区分待办、进行中和已完成；同一 turn 的完整快照原位更新并持久化，刷新页面后仍显示最新版本
...
- **消息编辑分支** — Codex 已发送的普通用户消息、运行中注入消息和初始 Prompt 均可直接点铅笔编辑；普通消息从上一 turn 精确 Fork，注入消息从其所在 turn 开头重放用户输入并替换该注入。旧指令与新指令各自保留后续 context，可在消息下方用左右箭头随时切换；实现用的分支 Task 不会在左侧列表重复显示
- **Codex Side 对话与过程折叠** — 每条已完成的助手回复下方可从该原生 turn 精确打开独立悬浮 Side 对话；主 Task 运行时也可从更早的回复分叉，窗口可最小化、刷新恢复或永久删除，且不会进入普通 Task 列表或污染主 context。已完成 turn 的 commentary、thinking、工具调用和 compact 标志自动收进可展开的 Activity 区，运行中的过程与期间注入的用户消息保持可见；选中助手文本可一键引用到当前输入框继续询问
